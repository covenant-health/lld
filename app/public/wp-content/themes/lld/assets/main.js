...
$(document).ready(function () {
  ...
  $('.cff-header').remove(); // fade in/out for scroll to top button
  ...
  $('a[href^="#"]').attr('data-scroll', ''); // clean up some junk

  $('article figure').removeAttr('style');
  $('article img').removeAttr('width').removeAttr('height'); // set focus to the search bar when it's been exposed
  ...
  $('.header-text').removeAttr('style'); // add class to figures that contain
...
$(window).on('load resize', function sidebarFix() {
  ...
  var mqsm = window.matchMedia('screen and ( max-width: 991px )'); // Save the heading text. We'll need that in a sec.

  var headingText = $('.nav-sidebar h3, .nav-sidebar button').text(); // clear the timeout so we can start fresh
  ...
  timer = setTimeout(function () {
    if (false === mqsm.matches) {
      ...
      $('.nav-sidebar button').replaceWith('<h3>' + headingText + '</h3>'); // Fix the menu

      $('.nav-sidebar ul').removeClass('dropdown-menu').addClass('nav sidenav-menu');
    } else {
      // wrap the list in a container div
      $('.nav-sidebar').wrapInner('<div class="dropdown"></div>'); // Replace the class in the dropdown menu

      $('.nav-sidebar ul').removeClass('nav sidenav-menu').addClass('dropdown-menu').attr('aria-labelledby', 'sidebar-nav-btn'); // Change the heading to a button
...
var scroll = new SmoothScroll('a[href*="#"]', {
  ...
  popstate: true,
  // Animate scrolling with the forward/backward browser buttons (requires updateURL to be true)
  // Custom Events
